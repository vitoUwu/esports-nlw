  ...
  useEffect(() => {
	...
	return (
		<Dialog.Portal>
			...
			<Dialog.Content className='fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] drop-shadow-lg shadow-black/25'>
				...
				<form onSubmit={handleCreateAd} className='mt-8 flex flex-col gap-4'>
					<div className='flex flex-col gap-2'>
						...
						<select name='game' id='game' className='bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 appearance-none' defaultValue=''>
							<option disabled>Selecine o game que deseja jogar</option>
					...
					<div className='grid grid-cols-2 gap-6'>
						...
						<div className='flex flex-col gap-2'>
							...
							<Input name="discord" id="discord" type="text" placeholder='usuário#0001' />
						</div>
					</div>
					...
					<label className='mt-2 flex items-center gap-2 text-sm select-none'>
						<Checkbox.Root checked={useVoiceChannel} onCheckedChange={(checked) => setUseVoiceChannel(checked === 'indeterminate' ? false : checked)} className='w-6 h-6 p-1 rounded bg-zinc-900'>
							<Checkbox.Indicator>
